...
zh.data = {
  ...
  MSG_signature:        '签名',
  MSG_verify:           '验证消息',
  MSG_info1:            '包括当前日期，以便签名不能在不同的日期重复使用。',
  MSG_info2:            '包括您的昵称和使用昵称的位置，以便其他人不能使用。,
  MSG_info3:            '包括消息的特定原因，因此不能重复用于不同的目的。',

  ERROR_22:             '无法估计gas。帐户中没有足够的资金，或接收合同地址会抛出错误。随意手动设置gas并继续。发送时的错误消息可能更多信息。',

  /* Mnemonic Additions */
  MNEM_1:               '请选择您要与之互动的地址。',
  MNEM_2:               '您的单个HD助记词组可以访问多个钱包/地址。请选择您要与此互动的地址。',
  MNEM_more:            '更多地址',
  MNEM_prev:            '上一个地址',
  x_Mnemonic:           '助记短语 (MetaMask / Jaxx / ether.cards)',
  ADD_Radio_5:          '粘贴/输入你的助记符',
  SEND_custom:          '添加自定义令牌',
  ERROR_21:             ' 不是有效的ERC-20代币。如果其他代币正在加载，请删除此代币，然后重试.',
  TOKEN_show:           '显示所有代币',
  TOKEN_hide:           '隐藏代币',
  WARN_Send_Link:       '您通过具有地址，值，gas，数据字段或交易类型（发送模式）的链接到达。您可以在发送之前更改任何信息。解锁你的钱包开始。',
  WARN_Send_Link_2:     '**警告：**如果通过"ETH（标准交易）"发送，您只能包括数据。请从URI中删除"sendMode"和/或"tokenSymbol"以发送带有数据的事务。',

  /* Hardware wallets */
  x_Ledger:             '硬件钱包Ledger Nano S',
  ADD_Ledger_1:         '连接你的 Ledger Nano S',
  ADD_Ledger_2:         '打开以太坊应用程序 (或一个智能合约应用DAPP)',
  ADD_Ledger_3:         '确定这个浏览器在设置中已经启用了这个支持',
  ADD_Ledger_4:         '如果在设置中找不到浏览器支持，请验证您是否拥有 [Firmware >1.2](https://www.ledgerwallet.com/apps/manager)',
  ADD_Ledger_0a:        '通过一个安全链接（SSL）重新打开MyEtherWallet',
  ADD_Ledger_0b:        '用[Chrome](https://www.google.com/chrome/browser/desktop/) 或 [Opera](https://www.opera.com/)重新打开MyEtherWallet ',
  ADD_Ledger_scan:      '连接到Ledger Nano S',

  /* Parity Error Messages */
  PARITY_AlreadyImported:       "已导入具有相同散列的事务。",
  PARITY_Old:                   "事务随机数太低。尝试增加随机数。",
  PARITY_TooCheapToReplace:     "交易费太低。在队列中有另一个事务具有相同的临时值。尝试增加费用或增加随机数",
  PARITY_LimitReached:          "队列中的事务过多。您的交易由于限制而被删除。尝试增加费用。",
  PARITY_InsufficientGasPrice:  "交易费太低。它不能满足您节点的最低费 (最小: {}, 获得: {}). 尝试增加费用",
  PARITY_InsufficientBalance:   "不充足的资金。您尝试发送交易的帐户没有足够的资金。需要 {} 并获取到: {}.",
  PARITY_GasLimitExceeded:      "交易成本超过当前的气体限额。 限制: {}, 获取到: {}. 尝试减少供应gas",
  PARITY_InvalidGasLimit:       "供应的gas超过极限",
  ...
  HELP_Warning:       '如果你在**Dec. 31st, 2015**之前已创建一个钱包或下载了repo，请检查您的钱包＆amp;下载repo的新版本。点击查看详情。',
  HELP_Desc:          '看到了什么东西缺失？还有另一个问题？[请与我们联系](mailto:myetherwallet@gmail.com), 并且我们不仅会回答您的问题，我们将更新此页面，以对未来的人更有用！',
  HELP_Remind_Title:  '一些提醒',
  HELP_Remind_Desc_1: '**以太坊，MyEtherWallet.com和MyEtherWallet CX，我们使用的一些基础Javascript库正在积极发展。**虽然我们已经彻底测试，成千上万的钱包已经成功地创建了世界各地的人，总是有一些意想不到的事情，导致你的ETH丢失的可能性。请不要投资超过你愿意输，请小心。如果有事情发生，我们很抱歉，但 **我们不负责丢失的以太**.',
  HELP_Remind_Desc_2: 'MyEtherWallet.com和MyEtherWallet CX不是“网钱包”。你不创建一个帐户或给我们你的以太。所有数据永远不会离开您的计算机/您的浏览器。我们让您轻松创建，保存和访问您的信息并与区块链进行交互。',
  HELP_Remind_Desc_3: '如果您不保存私钥和密码，则无法恢复对您的钱包或其持有的资金的访问。将其备份在多个物理位置&ndash;不只是在你的电脑！',

  HELP_0_Title:        '0)我是一个新手，我需要做什么？',
  HELP_0_Desc_1:       'MyEtherWallet让你能够生成新的钱包，所以你可以存储你自己，而不是在交换。此过程完全在您的计算机上进行，而不是我们的服务器。因此，当您生成新的钱包时，**你负责安全地备份它**.',
  HELP_0_Desc_2:       '创建一个新的钱包',
  HELP_0_Desc_3:       '备份钱包',
  HELP_0_Desc_4:       '验证您是否有权访问此新钱包，并已正确保存所有必要信息。',
  HELP_0_Desc_5:       '将Ether传输到此新钱包。',

  HELP_1_Title:        '1) 我怎么创建一个新钱包？ ',
  HELP_1_Desc_1:       '转到“生成电子钱包”页面',
  HELP_1_Desc_2:       '转到“添加电子钱包”页面，然后选择“生成新电子钱包”',
  HELP_1_Desc_3:       '输入强密码。如果你认为你可能会忘记它，将它保存在某个安全的地方。您将需要此密码才能发送交易。',
  HELP_1_Desc_4:       '点击 "生成".',
  HELP_1_Desc_5:       '您的钱包现已生成。',

  HELP_2a_Title:       '2a) 如何保存/备份我的钱包？',
  HELP_2a_Desc_1:      '您应该始终在外部并在多个物理位置备份您的钱包 - 例如在USB驱动器和/或一张纸上。',
  HELP_2a_Desc_2:      '保存地址。你可以保持自己或与他人分享。这样，其他人可以转移以太给你。',
  HELP_2a_Desc_3:      '保存私钥的版本。不要与任何其他人分享。当你想访问你的以太网发送它时，你的私钥是必要的！有3种类型的私钥：',
  HELP_2a_Desc_4:      '将您的地址，私钥的版本和纸质钱包的PDF版本放在一个文件夹中。将其保存在计算机和USB驱动器上。',
  HELP_2a_Desc_5:      '如果您有打印机，请打印钱包。否则，请在一张纸上写下您的私钥和地址。将其存储为安全位置，与计算机和USB驱动器分开。',
  HELP_2a_Desc_6:      '请记住，您必须防止丢失的密钥和密码，由于您的硬盘驱动器故障，或USB驱动器或一张纸丢失或故障。你还必须记住整个地区的身体损失/损害（想想火或洪水）。',

  HELP_2b_Title:       '2b) 如何使用MyEtherWallet安全/离线/冷藏？',
  HELP_2b_Desc_1:      '进入 [https://github.com/kvhnuke/etherwallet/releases/latest](https://github.com/kvhnuke/etherwallet/releases/latest).',
  HELP_2b_Desc_2:      '点击`dist-vX.X.X.X.zip`.',
  HELP_2b_Desc_3:      '移动zip包到电脑的空间里',
  HELP_2b_Desc_4:      '解压缩并双击`index.html`.',
  HELP_2b_Desc_5:      '生成具有强密码的钱包。',
  HELP_2b_Desc_6:      '保存地址。保存私钥的版本。保存密码，如果你可能不记得它永远。',
  HELP_2b_Desc_7:      '将这些纸/ USB存储在多个物理上分开的位置。',
  HELP_2b_Desc_8:      '转到“查看电子钱包信息”页面，输入您的私钥/密码，以确保它们是正确的，并访问您的钱包。检查您记录的地址是否相同。',
  ...
  HELP_18_Title:          '18) 我的geth钱包文件在哪里？',

  HELP_19_Title:          '19) 我的Mist钱包在哪里？ ',
  HELP_19_Desc_1:         'Mist文件通常位于上面的文件位置，但打开Mist更容易，在顶部栏中选择“帐户”，选择“备份”，然后选择“帐户”。这将打开存储文件的文件夹。',
  ...
  HELP_SecCX_Title:       '安全项 - MyEtherWallet CX ',
  HELP_SecCX_Desc_1:      '此扩展程序将我的信息保存在哪里？',
  HELP_SecCX_Desc_2:      '您存储在此Chrome扩展程序中的信息将通过 [chrome.storage](http://chrome.storage/)保存。 - 这是您在Chrome中保存密码时保存密码的地方。',
  HELP_SecCX_Desc_3:      '什么信息已被保存？ ',
  HELP_SecCX_Desc_4:      '地址、昵称和私钥存储在chrome.storage中。使用您添加钱包时设置的密码加密私钥。昵称和钱包地址未加密。',
  ...
  HELP_Sec_Desc_4:        '检查URL - 此网站通过GitHub提供，您可以在这里看到源代码： [https://github.com/kvhnuke/etherwallet/tree/gh-pages](https://github.com/kvhnuke/etherwallet/tree/gh-pages) 到 [https://www.myetherwallet.com](https://www.myetherwallet.com).',
  HELP_Sec_Desc_5:        '生成钱包，您可以下载 [源代码并在本地运行](https://github.com/kvhnuke/etherwallet/releases/latest). 查看 #5 以上.',
  HELP_Sec_Desc_6:        '生成测试钱包并检查并查看正在发生的网络活动。最简单的方法是右击页面并点击“检查元素”。转到“网络”选项卡。生成测试钱包。您将看到没有网络活动。您可能会看到类似数据的图像：image / gif和data：image / png。这些是QR码正在生成...在您的计算机上...由您的计算机。未传输任何字节。',
  HELP_Sec_Desc_7:        '现在，为了安全起见，移到"资源"页面。在这里，您可以看到组成网站的所有不同的元素。如果您单击本地存储，Cookie和缓存，您将看到没有存储任何内容。没有什么被保存了。刷新页面并且你将回到首页。',
  HELP_Sec_Desc_8:        '如果你不舒服使用这个工具，那么通过一切手段，不要使用它。我们创建这个工具是一个有用的方式，人们生成钱包和交易，而不需要潜入命令行或运行一个完整的节点。再次，如果您有任何疑虑，请随时与我们联系，我们将尽快回复。谢谢！',

  HELP_FAQ_Title:         '常见问题的更多有用回答',
